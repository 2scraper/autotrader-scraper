...
**"autotrader-scraper security"** in the subject — otherwise it lands in a queue
...
- **Bypassing autotrader.com's bot protection.** This scraper drives an
  ordinary, unmodified browser and is served or refused the way a browser
  is. Anything about how Akamai behaves is not a vulnerability in this
  repository.
- **The scraper stopped working.** autotrader.com changing its pages is
...
- **raw HTML dumps** (`--dump-html`, and the automatic `_debug.html` on a
  refused or unparseable page) — on this site a served page's
  `__NEXT_DATA__` carries the address that fetched it (`clientIp`) and a
  session id (`atcCookie`). Do not commit or share one; `make_fixtures.py`
  shows what has to go before a capture is public.
