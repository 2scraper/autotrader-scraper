...
diff_runs.py
------------
Diff two autotrader-scraper JSON outputs of the same mode by `sku`.
...
What each means depends on the mode, and it is worth being exact:
    ...
    listing   a car entering or leaving the slice the run fetched, or its
              price, mileage or deal rating moving. `removed` does NOT mean
              sold: the site serves 300 cars of a search however many
              matched, so a car can fall below that cut under the same
              ordering, and a run of fewer pages holds fewer still.
    vehicle   one row per listing id asked for. `removed` is a listing that
              has gone (the sidecar's `vehicles_gone` names them).
...
Refused, with --force as the escape hatch:
  ...
  * runs of different QUERIES (the search URL, its ordering or page size),
...
# car. `page` and `position` are here because a live search reorders itself
# between runs: a car moving from position 4 to 5 is the ordering, not the
# car.
#
# Three more are this site's: `days_on_site` grows by one every day for
# every car, so tracking it reports the calendar; `distance_miles` is
# measured from the SEARCH, not a fact about the car; and
# `premium_spotlight` is the seller's advertising package, which says
# nothing about the car either.
UNTRACKED_FIELDS = frozenset({
    "source", "scraped_at", "sku", "page", "position", "mode", "sort",
    "data_source", "url", "days_on_site", "distance_miles",
    "premium_spotlight",
})
...
def _headline(row: dict) -> str:
    ...
    price = row.get("price")
    return "%s  %s mi  %s" % ("$%s" % price if price is not None else "no price",
                              row.get("mileage"), row.get("seller_name") or "")
...
def _check_comparable(args) -> bool:
    ...
    if len(queries) == 2 and queries["--old"] != queries["--new"]:
        problems.append(
            ...
            f"{queries['--new']}). The site serves 300 cars of a search, so "
            f"the ordering decides WHICH cars a run holds at all, and a "
            f"different search is a different sample. Every line would "
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Diff two autotrader-scraper JSON outputs by sku.")
