...
# ===========================================================================
# What is deliberately NOT here, and what this repo measured
# ===========================================================================
# Measured on autotrader.com, 2026-09-24: no captcha of any kind stood in
# front of a results page or a detail page, served or refused. What the site
# sends a client it refuses is a static Akamai page ("page unavailable",
# HTTP 200), and that page DOES load a reCAPTCHA (v2, render=explicit): the
# widget on its own support form, which files an unblock request with the
# site's staff (an e-mail address and a message, POSTed to the site's
# `unblockrequest` endpoint). Solving it would submit a ticket, not let the
# run in, so page_flow classifies that page as `blocked`, a state it never
# solves, and the suite pins it.
#
# Akamai Bot Manager's own behavioural challenge (`sec-if-cpt-container`)
# was not seen either; this repo does not implement it.
#
# The reCAPTCHA / AWS WAF machinery above is kept, and that is a deliberate
# asymmetry rather than an inconsistency. Detection stays broad because
# which challenge a visitor meets depends on the exit and on what the
# address has been doing — a narrow list is how a challenge gets reported as
# an empty page months later. It is family core, verified on the sibling
# sites its comments name.
