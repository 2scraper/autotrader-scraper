...
# One real run per mode: two row classes, two samples.
SAMPLE_FILES = ("sample_output.json", "sample_output.csv",
                "sample_output_vehicle.json", "sample_output_vehicle.csv")
...
#
# autotrader.com names a listing's photos by a 32-hex file name on its own
# image host: `https://images.autotrader.com/hn/c/{32 hex}.jpg`. The row's
# `image_url` is that address, so every sample row carries one. It is a
# published image address, which the site puts in front of every visitor.
#
# The exemption is that exact host, path and extension, and nothing else. A
# bare 32-hex anywhere else still fails, including in the same file on
# another line (smoke_test plants one to prove it). The value was kept
# rather than removed (§24 prefers removal) because `image_url` is a real
# column: the one photo a results page carries.
#
# Checked rather than assumed: the listing ids, VINs, stock numbers and
# dealer ids this site uses are not hex, and the other image host it uses
# (`assets.cai-media-management.com`) names files by dashed UUIDs, which the
# \b-bounded rule does not match.
SITE_PUBLIC_IDS = (
    re.compile(r"images\.autotrader\.com/hn/c/[0-9a-f]{32}\.(?:jpe?g|png|webp)\b"),
)
...
# are photo addresses, and SITE_PUBLIC_IDS forgives exactly that context and
# nothing else, in every file.
...
CAPTURES_IN_HISTORY_DECIDED = {
    ...
    # be made quickly: 1.5-3 MB Next.js pages whose __NEXT_DATA__ holds the
    # REQUESTING address (`clientIp`, and `ip` in the analytics island), a
    # session id (`atcCookie`), and, for a private seller, a personal
    # `consumerId`. That is why the raw captures stay in an ignored
    # `captures/` and only make_fixtures.py's scrubbed cut is committed.
}
